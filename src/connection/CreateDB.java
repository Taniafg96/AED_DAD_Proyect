...
public class CreateDB{
    ...
    private void eliminar(){
        try (Connection connect = DriverManager.getConnection(DATABASE, user, password);
            ...
            db.executeUpdate("drop database if exists " + DATABASENAME);
    ...
    private void createTableProductos(){
        try (Connection connect = DriverManager.getConnection(DATABASE + DATABASENAME, user, password);
            ...
            db.executeUpdate("CREATE TABLE Productos(\n" +
                            ...
                            "    primary key (codigo),\n" +
                            "    foreign key (cod_almacen) references Almacen(codigo) on delete set null on update cascade\n" +
                            ")");
    ...
    private void createTableVentas(){
        try (Connection connect = DriverManager.getConnection(DATABASE + DATABASENAME, user, password);
            ...
            db.executeUpdate("CREATE TABLE Ventas(\n" +
                            ...
                            "    foreign key (DNI_Empleado) references Usuarios(dni) on delete set null on update cascade,\n" +
                            "    foreign key (codigo_producto) references Productos (codigo) on delete set null on update cascade,\n" +
